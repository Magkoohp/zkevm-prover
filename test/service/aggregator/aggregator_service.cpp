...
::grpc::Status AggregatorServiceImpl::Channel(::grpc::ServerContext* context, ::grpc::ServerReaderWriter< ::aggregator::v1::AggregatorMessage, ::aggregator::v1::ProverMessage>* stream)
{
    ...
    const string outputBlobInnerFile03 = "testvectors/blobOuterProof/blob_inner_recursive1.zkin.proof_03.json"; //note recursive 1 is necessary?

    const string inputBlobOuterFile03a  = outputBatchFile03;
    const string inputBlobOuterFile03b  = outputBlobInnerFile03;
    const string outputBlobOuterFile03  = "testvectors/aggregatedBlobOuterProof/blob_outer_recursive1.zkin.proof_01.json"; //note here is necessary a recursive 1 no??
    ...
    const string outputBlobInnerFile44 = "testvectors/blobOuterProof/blob_inner_recursive1.zkin.proof_44.json"; //note recursive 1 is necessary?

    const string inputBlobOuterFile44a  = outputBatchFile4;
    const string inputBlobOuterFile44b  = outputBlobInnerFile44;
    const string outputBlobOuterFile44  = "testvectors/aggregatedBlobOuterProof/blob_outer_recursive1.zkin.proof_44.json"; //note here is necessary a recursive 1 no??

    const string inputBlobOuterFile04a  = outputBlobOuterFile03;
    const string inputBlobOuterFile04b  = outputBlobOuterFile44;
    const string outputBlobOuterFile04  = "testvectors/finalProof/blob_outer_recursive2.zkin.proof_04.json"; //note here is necessary a recursive 1 no??
    ...
    for ( uint64_t loop=0; loop<AGGREGATOR_SERVER_NUMBER_OF_LOOPS; loop++ )
    {
        ...
        grpcStatus = GenAggregatedBlobOuterProof(context, stream, inputBlobOuterFile04a, inputBlobOuterFile04b, outputBlobOuterFile04);
        if (grpcStatus.error_code() != Status::OK.error_code())
        {
            return grpcStatus;
        }
